# Danh sách tài liệu cần xã cung cấp

**Dự án:** Tra cứu thủ tục hành chính - UBND xã Phú Thành
**Ngày tạo:** 2026-08-11
**Trạng thái:** Đang chờ

---

## 1. Logo và hình ảnh nhận diện

| # | Tài liệu | Định dạng | Kích thước đề xuất | Ghi chú |
|---|----------|-----------|-------------------|---------|
| 1.1 | Logo UBND xã Phú Thành | PNG hoặc SVG | 200x200px trở lên | Nền trong suốt |
| 1.2 | Logo tỉnh Đồng Tháp (nếu có) | PNG hoặc SVG | 200x200px trở lên | Tùy chọn |
| 1.3 | Hình ảnh xã (nếu muốn làm nền) | JPG/PNG | 1920x1080px | Tùy chọn |

---

## 2. Thông tin văn bản

| # | Nội dung | Ví dụ | Ghi chú |
|---|----------|-------|---------|
| 2.1 | Tên đầy đủ của xã | "UBND xã Phú Thành" | Xác nhận chính tả |
| 2.2 | Địa chỉ đầy đủ | "Huyện Tam Nông, tỉnh Đồng Tháp" | |
| 2.3 | Số điện thoại liên hệ | (Bản thiết kế mẫu: 0277 3827 456) | **Cần xác nhận số thật** |
| 2.4 | Giờ làm việc chính thức | "7h30 - 11h30, 13h30 - 17h00" | Để hiện trên giao diện |
| 2.5 | Ngày nghỉ trong tuần | "Thứ 7 và Chủ nhật" | |

---

## 3. Xác nhận nội dung thủ tục

| # | Nội dung | Trạng thái |
|---|----------|------------|
| 3.1 | Danh sách 294 thủ tục từ QĐ 253/QĐ-UBND-HC | ⏳ Đang nhập |
| 3.2 | Xác nhận "Người dân thường hỏi" - 4 thủ tục phổ biến | ⏳ Chờ xã chọn |
| 3.3 | Tên thân thiện cho từng ngành | ⏳ Chờ xác nhận |

---

## 4. Tên thân thiện đề xuất (chờ xã duyệt)

| Ngành chính thức | Tên thân thiện đề xuất | Xã duyệt |
|-----------------|----------------------|----------|
| Nông nghiệp và Môi trường | Đất đai & Nông nghiệp | ☐ |
| Tư pháp | Hộ tịch & Giấy tờ | ☐ |
| Xây dựng | Xây nhà & Sửa nhà | ☐ |
| Tài chính | Thuế & Lệ phí | ☐ |
| Nội vụ | Tổ chức & Nhân sự | ☐ |
| Y tế | Sức khỏe & Y tế | ☐ |
| Văn hóa, Thể thao và Du lịch | Văn hóa & Thể thao | ☐ |
| Công Thương | Kinh doanh & Buôn bán | ☐ |
| Quốc phòng | Nghĩa vụ quân sự | ☐ |
| Thanh tra | Khiếu nại & Tố cáo | ☐ |
| Cư trú | Đăng ký cư trú | ☐ |
| Liên thông điện tử | Đăng ký trực tuyến | ☐ |

---

## 5. Duyệt tên thân thiện (ten_than_thien)

Các tên dưới đây do tư vấn đề xuất, CHƯA được xã duyệt.
Cần cán bộ Một cửa xác nhận dân có thực sự hỏi như vậy không,
hoặc sửa lại cho đúng cách dân nói.

| Mã TTHC | Tên chính thức | Tên thân thiện đề xuất | Xã duyệt |
|---------|----------------|----------------------|----------|
| 2.001263 | Đăng ký việc nuôi con nuôi trong nước | Nhận con nuôi | ☐ |
| 2.001255 | Đăng ký lại việc nuôi con nuôi trong nước | Đăng ký lại việc nhận con nuôi | ☐ |

Bảng này cập nhật thêm khi có thủ tục mới.

---

## 6. Thông tin phần cứng kiosk

| # | Câu hỏi | Trả lời |
|---|---------|---------|
| 6.1 | Máy nào dùng làm kiosk: mua mới hay tận dụng máy sẵn có? | |
| 6.2 | Màn hình bao nhiêu inch, độ phân giải bao nhiêu? | |
| 6.3 | Đặt ngang hay dựng dọc? | |

**Lưu ý:** Giao diện yêu cầu tối thiểu 1920×1080, hướng ngang. Xem chi tiết tại `03_Kiosk/yeu-cau-phan-cung.md`.

---

## 7. Checklist xác nhận

- [ ] Logo đã gửi
- [ ] Thông tin văn bản đã xác nhận
- [ ] Danh sách "thường hỏi" đã chọn
- [ ] Tên thân thiện cho ngành đã duyệt
- [ ] Tên thân thiện cho thủ tục đã duyệt
- [ ] Giờ làm việc đã xác nhận

---

**Liên hệ kỹ thuật:** [Tên developer]
**Ghi chú:** Vui lòng gửi file qua email hoặc Zalo. Ưu tiên định dạng gốc (vector) cho logo.
